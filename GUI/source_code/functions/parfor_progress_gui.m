function percent = parfor_progress_gui(handles, caller, N)
...
%Modified by Arthur to work together with the waitbar
%For help, contact:
%              Arthur Chakwizira
...
switch caller
    case 'bzd'
        caller_id = 'Bezier-curve deconvolution: ';
    case 'svd'
        if handles.do_oSVD; caller_id = 'oSVD deconvolution: '; end
        if handles.do_cSVD; caller_id = 'cSVD deconvolution: '; end
        if handles.do_sSVD; caller_id = 'sSVD deconvolution: '; end
end
...
elseif N == 0
    ...
    if nargout == 0
        waitbar(1, p_bar, [caller_id num2str(percent) '% complete'])
        close(p_bar)
    ...
    if nargout == 0 && round(percent) <= 100    
        waitbar(percent/100, p_bar, [caller_id num2str(round(percent)) '% complete'])        
