function [mean_vof_c, vof_area] = find_vof_gui(handles)
...
title(handles.axes1, ['Concentration: Slice number ' num2str(vof_slice)])
...
wrap_text = 'Use slider to select slice for VOF selection. Push Continue when done.';
...
wrap_text = ['Click on image to select VOFs. Click once to activate cursor.'...
    'Clicking on selected pixel will deselect it.   Press:  S/Enter to save, R/Spacebar to redo selection, C to continue with current VOF,'...
...
blink_this(handles.edit7, 'b')    
    ...
    handles.vof_c = NaN(n_clicks, img_size(4));
    handles.vof_plots = zeros(n_clicks,1);
    handles.xcoords = zeros(n_clicks,1);
    handles.ycoords = zeros(n_clicks,1);
    handles.mean_vof_plot = plot(0, 0, 'k-','LineWidth', 2, 'Parent', handles.axes2);
    ...
    for i = 1:n_clicks
        ...
        if exit_now
            if this_key == 115 || this_key == 13 %s or enter
                ...
                wrap_text = ['Mean VOF saved. VOF selection still active. '...
                    'Press:  S/Enter to save, R/Spacebar to redo selection, C to continue with current VOF,'...
                    ' Esc to terminate selection.'];
                set(handles.edit7, 'String', wrap_text, 'ForegroundColor', 'b')
                blink_this(handles.edit7, 'b')
                else
                    wrap_text = ['Mean VOF not saved. VOF selection still active. '...
                    'Press:  S/Enter to save, R/Spacebar to redo selection, C to continue with current VOF,'...
                    'Esc to terminate selection.'];
...
%                     blink_this(handles.edit7, 'r')                    
            ...
            if this_key == 99
                ...
                save_data.this_folder = path_to_vof; save_data.this_format = 0;
                save_data.this_name = 0; save_data.data_to_save = mean_vof_c; save_this_file(save_data)
                wrap_text = 'VOF selection complete.';
                set(handles.edit7, 'String', wrap_text, 'ForegroundColor', 'b')
                end
                return
            end %c             
        ...
        if already_clicked && this_pixel ~= false
            set(handles.vof_plots(this_pixel), 'XData', [], 'YData', [])
            set(handles.roi_plots(this_pixel), 'XData', [], 'YData', [])
             handles.vof_c(this_pixel, :) = NaN(1, img_size(4));
        ...
        handles.vof_c(i,:) = dsc_data_c(pixel_x, pixel_y, current_slice, :);
        handles.vof_plots(i) = plot(t, handles.vof_c(i, :),'LineWidth', 0.8, 'HandleVisibility', 'off', 'Parent', handles.axes2);
        this_color = get(handles.vof_plots(i), 'color');
        ...
        handles.mean_vof_c = nanmean(handles.vof_c(1:i, :), 1);
        set(handles.mean_vof_plot, 'XData', t, 'YData', handles.mean_vof_c)
        drawnow
        guidata(hObject, handles)
        no_error = true;
    end
