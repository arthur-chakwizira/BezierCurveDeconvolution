function find_aif_semi_auto_gui(handles)
...
if (isempty(pixel_xcoords) || numel(pixel_xcoords) < 5) %@@@@@
    ...
    aif_c = NaN(size(pixel_xcoords, 1),  img_size(4));
    aif_s = NaN(size(pixel_xcoords, 1),  img_size(4));
    ...
    legend(handles.axes2, ["Mouse-tracking", "Mean AIF"])
    hold(handles.axes2, 'on')
    ...
    best_aif = mean(candidates, 1); %take average.
    ...
    handles.numaifs = numaifs;
    handles.ready_to_edit = true; %used to inform edit function that no clicks have been made yet
end

actice_rois = handles.rois(ishandle(handles.rois)); %make space for editing aif suggestions
...
if isfield(handles, 'aif_pixel_plots')&&isfield(handles, 'opt_aif_slices')
     set(handles.aif_pixel_plots(handles.opt_aif_slices == index), 'Parent', handles.axes1); 
     pause(0.1)
end
guidata(hObject, handles)
end
...
handles = guidata(hObject);
if ~ishandle(handles.aif_conc_image)||~isfield(handles, 'aif_candidates'); return; end
C = get(handles.axes1, 'CurrentPoint');
x = round(C(1,1)); %these are flipped
y = round(C(1,2));
pixel_x = x;
...
if isfield(handles, 'ready_to_edit')
    handles.aif_candidates_copy = handles.aif_candidates;
    handles = rmfield(handles, 'ready_to_edit'); %we only want to get a fresh copy of suggested AIFs once.
end

exist_s = false;
this_indx = false;
for c = 1:handles.numaifs %check if points have already been clicked
    if  handles.opt_aif_coordinates(c, 1) == pixel_x &&  handles.opt_aif_coordinates(c, 2) == pixel_y
        exist_s = true;
        this_indx = c;
    end
end
...
if exist_s && this_indx ~= false %check if the clicked pixel is one of our optimal pixels
    ...
    if ~isempty(data_on_pixel) %if it does
        ...
        set( handles.aif_plots(this_indx), 'XData', [], 'YData', []) %and delete the corresponding aif curve
         handles.aif_candidates_copy(this_indx, :) = NaN(1, handles.img_size(4)); %remove the curve from the list of optimal ones
        handles.best_aif = nanmean(handles.aif_candidates_copy, 1); %update the mean
        ...
        set( handles.aif_plots(this_indx), 'XData', handles.t, 'YData', handles.aif_candidates(this_indx, :)) %restore its aif curve
         handles.aif_candidates_copy(this_indx, :) = handles.aif_candidates(this_indx, :); %restore the curve to the list of optimal ones
        handles.best_aif = nanmean(handles.aif_candidates_copy, 1); %update the mean
        set(handles.best_aif_plot, 'XData', handles.t, 'YData', handles.best_aif) %and update the mean plot
        drawnow     %now
    end
