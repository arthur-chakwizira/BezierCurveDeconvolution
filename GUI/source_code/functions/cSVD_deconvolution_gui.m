function [r_svd, cbf_svd, delay_svd] = cSVD_deconvolution_gui(handles)
%        This function handles cSVD deconvolution. It does not display
...
ppm = ParforProgressbar(numIterations,'showWorkerProgress',false,...
    'progressBarUpdatePeriod',3,'title','cSVD deconvolution');
    ...
    for y = yrange
        for z = zrange
            if mask(x,y,z)
                ...
                Rut = cSVD_residue_function(C, mean_aif_c, tmp_t, Psvd);
...
%report completion
wrap_text = 'cSVD deconvolution complete';
