...
if ~handles.dsc_data_loaded
    wrap_text = 'OEF computation requires DSC data. Load data to proceed.'; %check that data exists
    set(handles.edit7, 'String', wrap_text, 'ForegroundColor', 'r') %inform user
    return
end

 %undo restrict analysis
 if isfield(handles, 'old_mask'); handles.mask = handles.old_mask; guidata(hObject, handles); end

 
wrap_text = 'Select folder containing fitted parameters';
set(handles.edit7, 'String', wrap_text, 'ForegroundColor', 'b')
if isfield(handles, 'file_folder'); default_folder = handles.file_folder; else; default_folder = ''; end
file_folder = uigetdir(default_folder, 'Select input data location.'); %invoke file-select dialog with this title
...
if handles.BzD
    ...
    cbf_fn = fullfile(file_folder, 'cbf_Bzd.nii');
    if ~isfile(cbf_fn);  wrap_text = 'Selected folder does not contain Bezier CBF.'; set(handles.edit7, 'String', wrap_text, 'ForegroundColor', 'r'); return; end
    fitd_cbf = niftiread(cbf_fn);
end


if handles.do_SVD
    r_svd_fn = fullfile(file_folder, 'fitd_r_svd.nii');
    if ~isfile(r_svd_fn);  wrap_text = 'Selected folder does not contain SVD residue functions.'; set(handles.edit7, 'String', wrap_text, 'ForegroundColor', 'r'); return; end
    fitd_r_svd = niftiread(r_svd_fn);
    cbf_fn = fullfile(file_folder, 'cbf_SVD.nii');
    if ~isfile(cbf_fn);  wrap_text = 'Selected folder does not contain SVD CBF.'; set(handles.edit7, 'String', wrap_text, 'ForegroundColor', 'r'); return; end
    fitd_cbf_svd = niftiread(cbf_fn);
end


%get needed data__________________________________________________________
target_folder =handles.target_folder;
if ~isfolder(target_folder); target_folder = generate_target_folder(handles); end
