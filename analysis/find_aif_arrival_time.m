function aif_arrival_time = find_aif_arrival_time(data, config)
%Author:
%‪André Ahlgren‬
%Arthur Chakwizira
...
% first we smooth the curve because edge detection methods are noise-sensitive
fwhm = 4; %filter witdh of kernel 
interpolFactor = 2; %linearly interpolate tissue curve and AIF with 2 points between each sample
intFactor = ((tl-1)*interpolFactor + tl); 
elements = 11;
