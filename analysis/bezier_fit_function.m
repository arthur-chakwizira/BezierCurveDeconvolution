function J = bezier_fit_function(p,t,ydata,para, Pnorm, noise,x0, with_delay, with_dispersion)

%this is the objective function for bezier curve deconvolution
c_aif       = para.c_aif;
aif_tail_c  = para.aif_tail_c;
kappa       = para.kappa;
te = para.te;

p = exp(p.*Pnorm); % "undo" normalisation before using parameters for next calculation
% p = (p.*Pnorm); % "undo" normalisation before using parameters for next calculation
...
omega = p(1:5); %always using order 3 curves
...
omega_s       = [   8   1   4  1   100  ]; %std of control points

f = p(6); %cbf

if f < 0; lambda1 = 1E6; end %reject negative cbf

f_s = 1E6; % = 60 ml/100g/min 

 omega_0       = [     8   0.5   2  0.0   15  ];
f_0 = 0.01;
 %means of prior distribution
% -------------------------------------------------------------------------
...
%convolve (delayed/dispersed) aif with residue function
y_m = kappa*f*dt.*filter(mean_aif_c,1,r); 

tmp_baseline_idx = 1:8;
tmp_tissue_s0 = mean(ydata(tmp_baseline_idx));
y_m = tmp_tissue_s0.*exp(-te*y_m);
