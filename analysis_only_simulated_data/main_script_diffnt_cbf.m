...
%Specifically for mass deconvolving simulated data without delay or
%disperison
...
%now no disp no delay
cbf_gray = [10, 20, 30, 40, 50, 60, 70];
cbf_white = cbf_gray;%./2;
% for sn_r = [20 100]
sn_r = 100;
for run = 1:1%length(cbf_gray)
config.target_folder = ['SIM-DATA-SNR-' num2str(sn_r) '-tight_sampling' '/junk/fminsearch_cubic' '/results_cbf_study_high_del_no_disp_cbf_' num2str(cbf_gray(run)) '_' num2str(cbf_white(run)) '_snr_' num2str(sn_r)];
% config.target_folder = ['SIM-DATA-SNR-' num2str(sn_r) '/junk/fminsearch_cubic_ONE_PRIOR' '/results_cbf_study_no_del_no_disp_cbf_' num2str(cbf_gray(run)) 'resfunc_linear' '_snr_' num2str(sn_r)];

path = ['SIM-DATA-SNR-'  num2str(sn_r) '-tight_sampling' '/different_cbf']; % intermediate_different_cbf boxcar_different_cbf  different_cbf
file = 'St_disp_0_exponential_lvl_high_delay_0_lvl_high_cbf_gw_10_5.nii';
% path = 'simulated_data';
% file = ['St_disp_0_exponential_lvl_high_delay_1_lvl_high_cbf_gw_' num2str(cbf_gray(run)) '_' num2str(cbf_white(run)) '.nii'];
% file = ['St_disp_0_exponential_lvl_high_delay_0_lvl_high_resfunc_linear_cbf_' num2str(cbf_gray(run)) '.nii'];
% lamda = 100;
% file = ['St_disp_0_exponential_lvl_high_delay_0_lvl_high_resfunc_gamma_dist_lambda_' num2str(lamda) '_cbf_' num2str(cbf_gray(run))  '_' num2str(cbf_white(run))  '.nii']; % '_' num2str(cbf_white)

path_to_data = fullfile(path, file);
% path_aif = path;
path_aif = ['SIM-DATA-SNR-' num2str(sn_r) '-tight_sampling']; %'-tight_sampling'
file_aif = 'aif.nii';
...
config.slice_range = [1 3 1 3 1 1]; 
...
vol = 1;
tim = 1;
%['results_v' num2str(vol) 't' num2str(tim)];
